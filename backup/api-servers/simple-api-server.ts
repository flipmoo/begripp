...
// Utility functions
import { getWeekDates, normalizeDate } from '../../utils/date-utils';
...
import { GrippApiClient, createApiRouter } from '../../db/unified/api';
...
/**
 * Unified data structure API
 *
 * Initialize the unified data structure API and mount it on /api/v1
 */
let unifiedApiRouter: express.Router | null = null;
...
getDatabase().then(async database => {
  ...
  // Initialize unified data structure API
  try {
    console.log('Initializing unified data structure API...');

    // Create unit of work with the database connection
    const unitOfWork = await createUnitOfWork(db);

    // Create API client
    const apiClient = new GrippApiClient({
      apiKey: process.env.GRIPP_API_KEY || '',
      apiUrl: process.env.GRIPP_API_URL || 'https://api.gripp.com/public/api/jsonrpc'
    });
    ...
    // Create API router
    unifiedApiRouter = createApiRouter(unitOfWork, apiClient, cacheManager);

    // Mount API router
    app.use('/api/v1', unifiedApiRouter);

    // API routes are now handled by the unified API router
    // No need to import additional routes

    console.log('Unified data structure API initialized successfully');
  } catch (error) {
    console.error('Failed to initialize unified data structure API:', error);
  }
}).catch(error => {
...
app.get('/api/employee-stats', async (req, res) => {
  try {
    ...
    console.log(`Fetching employee stats for year=${year}, week=${week}, isDashboard=${isDashboard}`);
    ...
    console.log(`Fetching employee data for week ${week} of ${year} (${formattedStartDate} to ${formattedEndDate})`);
    ...
    try {
      ...
      employees.forEach(employee => {
        ...
        if (employee.contract_startdate) {
          const contractStart = new Date(employee.contract_startdate);
          const contractEnd = employee.contract_enddate ? new Date(employee.contract_enddate) : null;

          // Check if contract is valid for this week
          const weekStart = new Date(formattedStartDate);
          const weekEnd = new Date(formattedEndDate);

          if (contractStart <= weekEnd && (!contractEnd || contractEnd >= weekStart)) {
            // Contract is valid for this week
            contractPeriod = contractEnd
              ? `${employee.contract_startdate} to ${employee.contract_enddate}`
              : `${employee.contract_startdate} to present`;
        ...
        // Calculate holiday hours
        const holidayHours = weeklyContractHours * 5; // Approximately 5 weeks vacation per year

        // Calculate expected hours for this week
        let expectedHours = weeklyContractHours;
        ...
        holidays.forEach(holiday => {
          ...
          // Only count weekdays (1-5 is Monday-Friday)
          if (dayOfWeek >= 1 && dayOfWeek <= 5) {
            // Reduce expected hours based on contract hours for that day
            const isEvenWeek = week % 2 === 0;
            let dayHours = 0;
        ...
        // Calculate actual hours (written + leave)
        ...
        // Store in map (to handle multiple contracts)
...
app.get('/api/invoices', async (req, res) => {
  try {
    ...
    if (year > 0) {
      ...
      // If no specific year, get all invoices (no date filter)
      console.log('No year filter applied, fetching all invoices');
    }
...
app.post('/api/v1/sync/invoices', async (req, res) => {
  try {
    ...
    // Fetch fresh invoice data from Gripp
    const response = await invoiceService.getAll();

    // Check if the response contains data
    if (!response || !response.result || !response.result.rows) {
      throw new Error('Failed to fetch invoices from Gripp API');
    }

    const invoiceCount = response.result.rows.length;
    console.log(`Successfully synced ${invoiceCount} invoices from Gripp API`);
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error syncing invoices'
    });
  }
});
