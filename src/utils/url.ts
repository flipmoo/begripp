...
/**
 * Generate a URL for the current week view
 */
export function generateCurrentWeekUrl(basePath: string = '/employees'): string {
  const now = new Date();
  const year = getYear(now);
  const week = getWeek(now, { weekStartsOn: 1, firstWeekContainsDate: 4 });
  
  return generateUrl(basePath, {
    year,
    week,
    viewMode: 'week'
  });
...
/**
 * Generate a URL for the current month view
 */
export function generateCurrentMonthUrl(basePath: string = '/employees'): string {
  const now = new Date();
  const year = getYear(now);
