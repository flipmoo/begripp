 ...
 */
export const MonthlyTargetForm: React.FC = () => {
  ...
  // Render een input veld voor een maand
  const renderMonthInput = (month: number, value: string) => {
    return (
      <div key={month} className="space-y-2">
        <Label htmlFor={`target-${month}`} className="font-medium">
          {monthNames[month - 1]}
        </Label>
        <input
          id={`target-${month}`}
          ...
          min="0"
          step="1000"
          className="flex h-10 w-full rounded-md border border-gray-300 bg-background px-3 py-2 text-sm focus:border-blue-500 focus:ring-blue-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
      ...
      <CardContent className="pt-6">
        <form id="monthly-targets-form" onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {formTargets.map(target => renderMonthInput(target.month, target.targetAmount))}
          </div>
