...
/**
 * DirectMonthlyTargetForm Component
 *
 * Een eenvoudige component voor het bewerken van maandelijkse targets,
 * met directe communicatie met de database.
 */
export const DirectMonthlyTargetForm: React.FC = () => {
  const { selectedYear, fetchMonthlyTargets } = useIris();
  const [targets, setTargets] = useState<Array<{ id?: number; month: number; targetAmount: string }>>([]);
  ...
  const fetchData = useCallback(async () => {
    setIsLoading(true);
    try {
      console.log(`Ophalen targets voor jaar ${selectedYear}`);
      const response = await fetch(`${API_BASE_URL}/api/v1/simple-targets?year=${selectedYear}`);
  ...
  const handleDirectUpdate = async (id: number, value: string) => {
    if (!id) return;
    
    try {
      console.log(`Direct update target ${id} naar ${value}`);
      
      const response = await fetch(`${API_BASE_URL}/api/v1/simple-targets`, {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        console.log('Target succesvol bijgewerkt:', data);
        // Geen toast hier om de gebruiker niet te storen
      } else {
        console.error('Fout bij bijwerken target:', data);
        toast({
          title: 'Fout bij opslaan',
          description: `Er is een fout opgetreden: ${data.message || 'Onbekende fout'}`,
          variant: 'destructive'
        });
      }
    } catch (error) {
      ...
      toast({
        title: 'Fout bij opslaan',
        description: 'Er is een fout opgetreden bij het opslaan van de target.',
        variant: 'destructive'
      });
    }
  };
  ...
  if (isLoading) {
    return (
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="text-center">Targets laden...</div>
      </div>
    );
  }
            ...
            {targets.map(target => (
              <div key={target.month} className="space-y-2">
                ...
                <input
                  ...
                  onBlur={(e) => target.id && handleDirectUpdate(target.id, e.target.value)}
